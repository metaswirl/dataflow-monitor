...
case class ModelFileWriter(folder: String, writeMetrics: Boolean = false) {
  ...
  inferredMetricNodeWriter.write("time;task;selectivity;inputRate;capacity;inQueue;outQueue;dropRate\n")
  ...
  def updateInferredMetrics(model : Model): Unit = {
    ...
    for (task <- model.tasks.values) {
      ...
      inferredMetricNodeWriter.write(f"$now;${task.id};${task.selectivity};${task.inRate};${task.capacity};${task.inQueueSaturation};${task.outQueueSaturation};${task.dropRate}\n")
    }
